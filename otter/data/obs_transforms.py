...
def decode_and_resize(
    ...
    for name in image_names:
        ...
        if name in resize_size:            
            ...
            # image = tf.image.resize(image, resize_size[name], method="lanczos3", antialias=True) # 224, W*224/H
            image = tf.image.resize(image, resize_size[name], method="bicubic", antialias=True) # 224, W*224/H
            # image = tf.image.crop_and_resize(image[None], [[0, 0, 1, 1]], [0], resize_size[name])
            # image = tf.image.resize_with_crop_or_pad(image, resize_size[name][0], resize_size[name][1])
