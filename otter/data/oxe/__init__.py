...
import os
from otter.data.oxe.oxe_dataset_configs import ActionEncoding, OXE_DATASET_CONFIGS
from otter.data.oxe.oxe_dataset_mixes import OXE_NAMED_MIXES, DATA_PATH
from otter.data.oxe.oxe_standardization_transforms_hbrl import OXE_STANDARDIZATION_TRANSFORMS, general_restructure
...
def make_oxe_dataset_kwargs(
    ...
    del dataset_kwargs['image_obs_keys']
    del dataset_kwargs['depth_obs_keys']

    dataset_kwargs["restructure"] = [
        OXE_STANDARDIZATION_TRANSFORMS[name],general_restructure]

    if force_recompute_dataset_statistics:
        dataset_kwargs["force_recompute_dataset_statistics"] = True
        
    data_folder = DATA_PATH[name]

    return {"name": name, "data_dir":os.path.join(data_dir,data_folder), **dataset_kwargs}
