...
def chunk_act_horizon(traj: dict, action_horizon: int = 1) -> dict:
    """Chunks actions into `action_horizon` current + future actions.

    The "action" key is given a new axis, making it of shape [traj_len, action_horizon, action_dim].
    The first axis indexes into the action chunk, containing the current action plus `action_horizon - 1`
    future actions.
    """
    ...
    if len(traj["action"].shape) == 2:
        ...
        gripper_distance = traj["observation"]["proprio"][...,-1]
        ...
        traj["action"] = delta_hand_action# [traj_len, action_horizon, action_dim]
    else:
...
def chunk_act_obs(
    ...
    nking schemes where an action may differ depending on which observation it is paired with.
    """
    traj_len = tf.shape(traj["action"])[0]
...
def subsample(traj: dict, subsample_length: int) -> dict:
    ...
    if traj_len > subsample_length:
        start_indice = tf.random.shuffle(tf.range(traj_len-subsample_length))[0]
        indices = tf.range(traj_len)[start_indice:start_indice+subsample_length]
        #indices = tf.random.shuffle(tf.range(traj_len))[:subsample_length]
        traj = tf.nest.map_structure(lambda x: tf.gather(x, indices), traj)

    else:
        indices = tf.range(subsample_length)
