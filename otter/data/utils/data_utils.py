...
def get_dataset_statistics(
    ...
    # else:
    #     dataset_iterator = dataset.prefetch(prefetch).as_numpy_iterator()
    ...
    if proprios:
        proprios = np.concatenate(proprios) + 1e-8
        metadata["proprio"] = {
...
def add_proprio_noise(traj: dict, noise: float) -> dict:
    """Adds Gaussian noise to the proprio field of a trajectory."""
    if "proprio" not in traj["observation"]:
        return traj

    traj["observation"]["proprio"] += tf.random.normal(
        tf.shape(traj["observation"]["proprio"]), stddev=noise
    )
    return traj
...
def normalize_action_and_proprio(
    ...
    if 'prev_action' in traj["observation"]:
        keys_to_normalize['prev_action'] = 'observation/prev_action'
...
def denormalize(traj, metadata, normalization_type, dataset_name):
    keys_to_denormalize = {
        "action": "action",
    }
    if "proprio" in traj["observation"]:
        keys_to_denormalize["proprio"] = "observation/proprio"
    
    metadata = metadata[dataset_name]
    if normalization_type == NormalizationType.NORMAL:
        # normalize to mean 0, std 1
        for key, traj_key in keys_to_denormalize.items():
            mask = metadata[key].get(
                "mask", np.ones_like(metadata[key]["mean"], dtype=np.bool_)
            )
            traj = dl.transforms.selective_tree_map(
                traj,
                match=lambda k, _: k == traj_key,
                map_fn=lambda x: np.where(
                    mask, x*metadata[key]["std"] + metadata[key]["mean"], x
                ),
            )
        return traj
    ...
    if normalization_type == NormalizationType.BOUNDS:
        ...
        for key, traj_key in keys_to_denormalize.items():
            mask = metadata[key].get(
                "mask", np.ones_like(metadata[key]["p01"], dtype=np.bool_)
            )
            traj = dl.transforms.selective_tree_map(
                traj,
                match=lambda k, _: k == traj_key,
                map_fn=lambda x: np.where(
                    mask,
                    (x + 1) * (metadata[key]["p99"] - metadata[key]["p01"]) / 2 + metadata[key]["p01"],
                    x,
                ),
            )
        return traj
