...
from otter.util.args import DatasetConfig, SharedConfig, LoggingConfig
...
def collate_fn_lambda(
    ...
    if tokenizer is not None:
        ...
        discrete_action = tokenizer(collated["action"][:, -1])  # List[List[int]]
        ...
        for i, action in enumerate(discrete_action):
            ...
            padded_action[i, :action_length] = torch.tensor(action)
            # Add EOS token right after the sequence
            padded_action[i, action_length] = eos_idx
