...
class MetricLogger(object):
    ...
    def log_every_epoch(self, iterable, print_freq, header=None):
        i = 0
        if not header:
            header = ''
    ...
    def log_every_step(self, iterable, print_freq, total_iters, header=None):
        i = 0
        if not header:
            header = ''
        start_time = time.time()
        ...
        for step in range(total_iters):
            ...
            if i % print_freq == 0:
                eta_seconds = iter_time.global_avg * (total_iters - i)
                eta_string = str(datetime.timedelta(seconds=int(eta_seconds)))
...
def resume_from_ckpt(args : ExperimentConfig, model_without_ddp, optimizer, loss_scaler):
    if args.shared_cfg.resume:
        ...
        model_without_ddp = load_state_dict_flexible(model_without_ddp, checkpoint)
